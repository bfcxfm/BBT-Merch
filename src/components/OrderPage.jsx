...
export default function OrderPage({
  ...
  const calculateTotalPrice = () => {
    ...
    currentDrink.toppings.forEach((topping) => {
      total +=
        (toppingPrices[topping.topping] || 0) *
        topping.quantity *
        currentDrink.quantity;
    });
    setTotalPrice(total);
  };
  ...
  const toggleTopping = (toppingId) => {
    setCurrentDrink((prevDrink) => {
      const toppingIndex = prevDrink.toppings.findIndex(
        (t) => t.topping === toppingId
      );
      let newToppings;
        ...
        <fieldset className="grid gap-6 rounded-lg border p-4">
            ...
            <div className="grid grid-cols-2 gap-5">
              {TOPPINGS.map((topping) => (
                <div key={topping.id} className="flex items-center space-x-2">
                  <Checkbox
                    id={`${topping.id}`}
                    checked={currentDrink.toppings.some(
                      (t) => t.topping === topping.id
                    )}
                    onCheckedChange={() => toggleTopping(topping.id)}
                  />
                  <Label htmlFor={`${topping.id}`}>{topping.name}</Label>
                  {currentDrink.toppings.some(
                    (t) => t.topping === topping.id
                  ) && (
                    <Input
                      type="number"
                      value={
                        currentDrink.toppings.find(
                          (t) => t.topping === topping.id
                        ).quantity
                      }
                      onChange={(e) =>
          ...
          <div className="grid gap-3">
            ...
            <Input
              ...
              onChange={(e) =>
                updateDrink("quantity", parseInt(e.target.value))
              }
              min={1}
            />
          </div>
